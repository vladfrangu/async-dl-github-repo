...
module.exports = input => {
	...
	const check = (header, options) => {
		...
		for (let i = 0; i < header.length; i++) {
			// If a bitmask is set
			if (options.mask) {
				// If header doesn't equal `buffer` with bits masked off
	...
	if (check([0x50, 0x4B, 0x3, 0x4])) {
		...
		if (check(oxmlContentTypes, { offset: 30 }) || check(oxmlRels, { offset: 30 })) {
			const sliced = buffer.subarray(4, 4 + 2000);
			const nextZipHeaderIndex = arr => arr.findIndex((el, i, arr2) => arr2[i] === 0x50 && arr2[i + 1] === 0x4B && arr2[i + 2] === 0x3 && arr2[i + 3] === 0x4);
			const header2Pos = nextZipHeaderIndex(sliced);

			if (header2Pos !== -1) {
				const slicedAgain = buffer.subarray(header2Pos + 8, header2Pos + 8 + 1000);
